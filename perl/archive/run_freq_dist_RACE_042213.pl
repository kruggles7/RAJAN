...
foreach my $filename2 ("RACE_bin", "RACE_bin_hispanic")
{
	...
	foreach my $filename (@files)
	{
		...
		close IN;
		open (OUT, ">freq_results/$filename.freq.txt") or die "can't open file";
		...
		if ($filename2 =~/\w/)
		{
			...
			close IN;
			open (OUT, ">freq_results/$filename.$filename2.freq.txt") or die "can't open file";
			for (my $k=0; $k<=$maxv2; $k++)
			{
				print OUT "\nPREVELANCE (PERCENTAGE)\t";
