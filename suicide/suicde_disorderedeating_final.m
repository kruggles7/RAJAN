...
for j=1:numel(ques1)
    ...
    for i=1:numel(ques2)
        ...
        for k=1:r
            ...
            index_girls1{k}=find(sex(k,:)==1);
            index_yes2{k}=find(qmat2(k,:)==1);
            index_girls2{k}=find(sex(k,:)==1);
        ...
        for k=1:r
            index_total_g1{k}=intersect(index_nomiss1{k},index_girls1{k}); %index of all girls who answered
            index_total_g2{k}=intersect(index_nomiss2{k},index_total_g1{k}); %index of all girls who answered
        end
%         for k=1:r
%             w=weight(k,:)'; 
%             total_girls1(k)=nansum(w(index_total_g1{k})); %total # of girls who answered
%             total_girls2(k)=nansum(w(index_total_g2{k})); %total # of girls who answered
%         end 
        for k=1:r
            index_yesgirls1{k}=intersect(index_yes1{k},index_girls1{k});
            index_yesgirls2{k}=intersect(index_yes2{k},index_girls2{k});
            yes_girls1(k)=nansum(index_yesgirls1{k});
            yes_girls2(k)=nansum(index_yesgirls2{k});
            yes_both{k}=intersect(index_yesgirls1{k},index_yesgirls2{k}); 
            yes_girls_both(k)=numel(yes_both{k}); 
            total_girls(k)=numel(index_total_g2{k}); 
        end
        for k=1:r
            plot_mat(i,k)=yes_girls_both(k)/total_girls(k)*100; 
        end 
    end 
    %make heatmap table
