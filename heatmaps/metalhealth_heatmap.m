...
for j=1:N
    ...
    if numel(indx)>0
        ...
        question_mat=question_mat(2:r,2:c); 
...
sex=sex(2:r,2:c); 
race=race(2:r,2:c); 
weight=weight(2:r,2:c); 

[r,c,z]=size(QUEST_MAT);
per_mat = zeros(16,r);
for i=1:r %start at second location becuase that is year 2003 (don't use 2001)

    index_yes{i}=find( QUEST_MAT(i,:,1)==1 & QUEST_MAT(i,:,2)==1 & QUEST_MAT(i,:,3)==1 & QUEST_MAT(i,:,4)==1 & QUEST_MAT(i,:,5)==1 );
    ...
    index_missQ{i}=find( QUEST_MAT(i,:,1)==9 | QUEST_MAT(i,:,2)==9 | QUEST_MAT(i,:,3)==9 | QUEST_MAT(i,:,4)==9 | QUEST_MAT(i,:,5)==9 ); %students who didn't answer the Q
    index_nomiss{i}=find( QUEST_MAT(i,:,1)<2 & QUEST_MAT(i,:,2)<2 & QUEST_MAT(i,:,3)<2 & QUEST_MAT(i,:,4)<2 & QUEST_MAT(i,:,5)<2 ) ; %answers that were NOT missing (ie. 0's and 1's / no's and yes's)
...
per_mat_map(1:15,1:c)=per_mat(1:15,1:c);
...
set (gca, 'FontSize',12); 
cd results
saveas (gcf,  'mental_health_all.fig' ); %can make pdf, jnp, or jpg 
print (gcf, '-dpng',    'mental_health_all.png'); 
cd ..
close all
