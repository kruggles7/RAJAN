 function [per_mat_map] = create_hm_graph_yeardiff( question_mat, filename )
...
if (graph==1)
	%year=2001:2:2011;	
	% create 3 'subplots': one for totals, one for girls, one for boy
	f1=subplot(3,1,1); 
	yes=per_mat(15,:);
	plot(year,yes, 'ko');
	hold on
...
if (heatmap==1)
    ...
    per_mat_map=per_mat_map/10; 
	h=figure; 
	[hImage]=heatmap_rb(per_mat_map, year, label_cell2, 1, M, 0, 'Colormap','money', 'UseLogColormap', false, 'ShowAllTicks',true, 'Colorbar',true,'TextColor','k', 'FontSize', 12); 
