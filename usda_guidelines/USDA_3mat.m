function []=USDA_3mat(sex, race, grade, question_mat, cutoff,question_mat2, cutoff2, question_mat3, cutoff3, label)

    for i=1:r
    %	total(i)=TOTAL(i,1);
        index_yes{i}=find(question_mat(i,:)>=cutoff & question_mat2(i,:)>=cutoff2 & question_mat3(i,:)>=cutoff3);
    ...
    for i=1:r
        index_missQ{i}=find(question_mat(i,:)== 0 & question_mat2(i,:)==0); %students who didn't answer the Q
        index_nomiss{i}=find(question_mat(i,:)>0 & question_mat2(i,:)>0); %answers that were NOT missing (ie. 0's and 1's / no's and yes's)
    end
    ...
    set (gca, 'FontSize',12); 
    print (gcf,'-dpng', [label '.png' ]);  %can make pdf, jnp, or jpg 
