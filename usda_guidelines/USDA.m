function []=USDA(sex, grade, race, weight, question_mat, cutoff, label)
    ...
    for i=1:r
        index_missQ{i}=find(isnan(question_mat(i,:))==1); %students who didn't answer the Q
        index_nomiss{i}=find(isnan(question_mat(i,:))==0); %answers that were NOT missing (ie. 0's and 1's / no's and yes's)
    end
