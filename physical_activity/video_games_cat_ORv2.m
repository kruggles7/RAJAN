...
for i=1:r
    ...
    k=cellfun(@isempty,OR_2013_(i,9)); %only 2013 data
    if (k==0)
        N=OR_2013_{i,9}; 
        final(i,1)=N; 
        C=OR_2013_CI_{i,9}; 
    ...
    if (k==0)
        ...
        lower(i,7)=str2double(dn_); 
        for p=(b(1)+1):numel(a)
            temp=a(p); 
            up_=[up_ temp]; 
        end 
        upper(i,7)=str2double(up_); 
    end 
end 
...
%     diff=max_-min_; 
%     diff2=0; 
%     diff3=final(i,2)-final(i,7); %slope
%     for j=2:7
%         for k=2:7
...
%     ylim([0 3]); 
%     set(gca, 'XTick', 1:7); 
%     set(gca, 'XTickLabel', {'0','>0', '>1', '>2', '>3', '>4', '>5'}); 
%     title(signif_pos{i,1}); 
%     if (i>1)
%         set(gca, 'YTick', []); 
%         linkaxes([h1 h2],'y'); 
...
%     
% end 
% saveas (gcf, 'Q72_changingOR_positivev2.fig'); 
% close all
% 
...
plot_mat=double.empty; 
for i=1:r
    min_=min(lower(i,2:7)); 
    if sum(final(i,:)>0) && min_>1
        high_OR{count,1}=qlabel{i,1}; 
        high_OR{count,2}=final(i,2); 
        high_OR{count,3}=final(i,7); 
        plot_mat(count,:,1)=final(i,:); 
        E=(upper(i,:)-lower(i,:))/2; 
        plot_mat(count,:,2)=E; 
        plot_mat(count,:,3)=lower(i,:); 
