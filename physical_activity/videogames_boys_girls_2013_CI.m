...
% conf_mat{1,4}='3-4 hours'; 
% conf_mat{1,5}='5+ hours'; 
% 
% conf_mat{1,6}='0 hours'; 
% conf_mat{1,7}='0.5-2 hours'; 
% conf_mat{1,8}='3-4 hours'; 
% conf_mat{1,9}='5+ hours'; 
% 
% conf_mat{1,10}='0 hours'; 
% conf_mat{1,11}='0.5-2 hours'; 
% conf_mat{1,12}='3-4 hours'; 
% conf_mat{1,13}='5+ hours'; 
% 
% conf_mat{1,14}='0 hours'; 
% conf_mat{1,15}='0.5-2 hours'; 
% conf_mat{1,16}='2-4 hours'; 
% conf_mat{1,17}='5+ hours'; 
% 
% conf_mat{1,18}='0 hours'; 
% conf_mat{1,19}='0.5-1 hours'; 
% conf_mat{1,20}='3-4 hours'; 
% conf_mat{1,21}='5+ hours'; 
% 
% conf_mat{2,1}='total'; 
% conf_mat{3,1}='girls';
% conf_mat{4,1}='boys';
...
%         elseif (j>4 && j<7) %3-4 hours hours or more
%             x(1,3)=total_yes(i); 
%             x(2,3)=yes_girls(i); 
%             x(3,3)=yes_boys(i); 
%             x(4,3)=yes_WG(i); 
...
%make plots!
x=2005:2:2013; 
lookat=[1, 2, 3]; %, 12, 13, 14, 15, 16, 17, 18, 19]; 
cmap=jet(numel(lookat)); 
title_mat={'0 hours', 'less than 3 hours', '3-4 hours', '5 or more hours'}; 
for j=1:4
    f2=subplot(1,4,j);
    for i=1:numel(lookat); 
        I=lookat(i); 
        y=[plot_mat(I,j), plot_mat(I, j+4), plot_mat(I,j+8), plot_mat(I,j+12), plot_mat(I,j+16)]; 
        plot(x, y,'-', 'MarkerFaceColor', cmap(i,:)); 
        hold on; 
    end 
    ylim([0 75]); 
    xlim([2004 2014]);
