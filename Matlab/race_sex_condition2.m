function [ girls_percent ] = race_sex_condition( question_mat, question_name, sex, race )
...
total=zeros(6,1); 
girls_mat=zeros(r,2,4); 
boys_mat=zeros(r,2,4); 
girls_percent=zeros(r,2,4); 
boys_percent=zeros(r,2,4); 
...
for i=1:r
    for j=2:c
        ...
        b=sex(i,j);   
        d=race(i,j)+1; 
        if ((race(i,j)~=9) && (question_mat(i,j)~=9))
            if b==1
...
%xlabel('Years'); 
hold off; 
legend ('B','W','H','O', 'Location', 'EastOutside'); 
j2=subplot(2,1,2); 
x=1:6; 
count_s=1; 
for j=1:4
...
hold off; 
legend ('B','W','H','O', 'Location', 'EastOutside'); 
print (gcf,'-dpng', [question_name '_RACE']);
