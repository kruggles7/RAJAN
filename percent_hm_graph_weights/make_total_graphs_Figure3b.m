...
%%%cocain in past month-----------------------------------------------
q4_mat=importdata('Q26--NaN.txt', '\t');
[r,c]=size(q4_mat); 
for i=1:r
%	total(i)=TOTAL(i,1);
...
%%%Q57 smoke at school-----------------------------------------------
q6_mat=importdata('Q57--NaN.txt', '\t');
[r,c]=size(q6_mat); 
for i=1:r
%	total(i)=TOTAL(i,1);
	index_yes{i}=find(q6_mat(i,:)==1);
    index_nomiss{i}=find(q6_mat(i,:)==0 | q6_mat(i,:)==1); %answers that were NOT missing (ie. 0's and 1's / no's and yes's)
end
...
for i=1:r
    ...
    total_ans(i)=nansum(w(index_nomiss{i})); 
    total_yes(i)=nansum(w(index_yes{i}));
    q6_total(i)=total_yes(i)/total_ans(i)*100; %total
end 
...
hold on
plot(x,q5_total,'--ks', 'MarkerFaceColor', 'k'); 
plot(x,q4_total, ':kd', 'MarkerFaceColor', 'k'); 
plot (x,q6_total,'-.k^','MarkerFaceColor', 'k');  
hold off
xlim([2000 2012]);
max3=max(q3_total); 
max4=max(q4_total); 
...
legend ('Carried a gun*', 'Has ever used methamphetamines', 'Smoked cigarettes in school*', 'Has used cocaine*', 'Location', 'SouthWest');  
xlabel('Year', 'FontSize',12); 
ylabel('% of Respondents Answering Positively', 'FontSize',12);  
set (gca, 'FontSize',12); 
%print (gcf,'-dmeta', 'Fig3b.emf' );