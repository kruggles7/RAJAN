...
for j=1:N
    ...
    cd results_020314
    cd NaN
    question_mat=importdata([questions{j} '--NaN.txt'], '\t');
    [r,c]=size(question_mat); 
    if r>2
        question_mat=question_mat(5:6,2:c); 
    else 
    ...
    %per_mat=flipud(per_mat); 
        ...
        cd results
        saveas (gcf, [questions{j} '_heatmap.fig']); %can make pdf, jnp, or jpg 
        print (gcf, '-dpng', [questions{j} '_heatmap.png']); 
        cd ..
        cd ..
        close all
end 
