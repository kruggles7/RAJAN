...
PO_overlap(:,4)=t(:,2); 
o=sum(PO_overlap,2); 
%PO only, PO+Heroin, PO+Inject, PO+heroin+inject
PO_overlap(1,:)=PO_overlap(1,:)/o(1)*100; %1 or 2 times
PO_overlap(2,:)=PO_overlap(2,:)/o(2)*100; %3-9 times
PO_overlap(3,:)=PO_overlap(3,:)/o(3)*100; %10-19 times
PO_overlap(4,:)=PO_overlap(4,:)/o(4)*100; %20-39 times
PO_overlap(5,:)=PO_overlap(5,:)/o(5)*100; %40+ times
...
H_overlap(:,4)=t2(:,2); 
o2=sum(H_overlap,2); 
%Heroin only, PO+Heroin, Heroin+Inject, Heroin+PO+inject
H_overlap(1,:)=H_overlap(1,:)/o2(1)*100; 
H_overlap(2,:)=H_overlap(2,:)/o2(2)*100; 
H_overlap(3,:)=H_overlap(3,:)/o2(3)*100; 
H_overlap(4,:)=H_overlap(4,:)/o2(4)*100; 
H_overlap(5,:)=H_overlap(5,:)/o2(5)*100; 
...
I_overlap(:,4)=inj(:,2); 
oi=sum(I_overlap,2); 
I_overlap(1,:)=I_overlap(1,:)/oi(1)*100; 
I_overlap(2,:)=I_overlap(2,:)/oi(2)*100; 

f1=subplot(2,1,1); 
bar(PO_overlap,'stacked'); 
ylim([0 100]);  
set (gca, 'FontSize', 12); 
ylabel('% of Total Rx Drug Users', 'FontSize', 12);
legend('Rx Drugs Only','Rx Drugs & Heroin', 'Rx Drugs & Injection Drugs', 'Rx Drugs, Heroin & Injection Drugs', 'Location','EastOutside'); 
f2=subplot(2,1,2); 
bar(H_overlap,'stacked'); 
ylim([0 100]);  
...
legend('Heroin Only','Heroin & Prescription Drugs', 'Heroin & Injection Drugs', 'Heroin, Prescription Drugs, & Injection Drugs', 'Location','EastOutside' ); 
set (gca, 'FontSize', 12); 
