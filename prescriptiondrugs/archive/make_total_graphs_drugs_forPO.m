...
max5=max(q5_total); 
max6=max(q6_total); 
maxT=max([max3; max4; max5; max6]); 
maxT=maxT+0.1*maxT; 
ylim([0 20]); 
set(gca, 'XTick', x); 
set(gca, 'XTickLabels', x); 

legend ('Ever used heroin', 'Ever used methamphetamines', 'Ever used ecstasy','Ever used cocaine', 'Location', 'NorthWest');  
xlabel('Year','FontSize', 12);  
ylabel('% of Respondents Answering Positively', 'FontSize', 12); 
set (gca, 'FontSize',12); 
print (gcf,'-dpng', 'Drug_trends_total.png' );