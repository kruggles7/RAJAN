...
%filename2
files2=dir(fullfile('C:','Users','kruggles7','Documents','MATLAB', 'CDC', 'data','cat_results_010314', '*.txt'));
N=length(files2);
for n=1:N
    ...
    if ( isnan(q2_RC)==0 && isnan(q1_RC)==0) % won't compare a question to itself
        ...
        [r1,c1]=size(quest_1);
        [r2,c2]=size(quest_2); 
        quest_2=quest_2(r2-1:r2,:);  

        r=r1;

        for i=1:r
            %Q1: 
                index_yes_1{i}=find(quest_1(i,:)>3); %students who answered yes to Q1
                index_no_1{i}=find(quest_1(i,:)==1); %students who answered no to Q1
                index_miss_1{i}=find(quest_1(i,:)==0); %students who didn't answer Q1
