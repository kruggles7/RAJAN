function [per_mat_map] = create_hm_graph( question_mat, filename, heatmap, graph )
...
if (graph==1)
	...
	saveas(gca, [ filename '_graph.pdf'] ); %can make pdf, jnp, or jpg 
	close 
end

if (heatmap==1)
	label_year={'2001', '2003','2005','2007','2009','2011'}; 
	label_cell2={'Total', 'Boys', 'Girls', 'Whites', 'Blacks', 'Hispanics', 'Other', 'White Boys', 'White Girls', 'Black Boys', 'Black Girls', 'Hispanic Boys', 'Hispanic Girls', 'Other Boys', 'Other Girls'}; 
	per_mat_map(1:15,1:r)=per_mat(1:15,1:r);
	per_mat_map=flipdim(per_mat_map,1);
	h=figure; 
	[hImage]=heatmap_rb(per_mat_map, label_year, label_cell2, 1, 25, 0, 'Colormap','money', 'UseLogColormap', false, 'ShowAllTicks',true, 'Colorbar',true,'TextColor','k', 'FontSize', 12); 
    %title (title1, 'FontSize', 12); 
	set (gca, 'FontSize',12); 
	print (gcf,'-dpdf', [ filename '_heatmap.pdf'] ;  %can make pdf, jnp, or jpg 
end
